# Petclinic Contrast Sample project

_Prepared 3 Apr 2020_

This project is based on the vulnerable Petclinic sample application from
Contrast Security, which is in turn based on the Petclinic sample application
distributed with the Spring framework from Pivotal.

The application is typically built using the `dobuild` script, which runs
Maven in a docker container.  If you already have Maven installed locally,
you can alternatively run the `mvnw` script to build using that.  The dobuild
script runs the targets "clean package".

Once built, you will need to copy in the Contrast agent JAR fie (named
`contrast.jar`) and, optionally, `contrast_security.yaml`.  The YAML file is
most often used to tell the agent how to connect to Team Server, but may
also contain other configuration options.  Note that the `run-petclinic`
script specifies the standalone_app_name option, which will override any such
option included in the YAML file.

The application can be run using the `run-petclinic` script.  This
will run Petclinic using a natively-installed JVM, but you can run the app
in a Docker container if you prefer.  Although no docker image or Dockerfile
is provided, you could use a standard Java base image and the -v and -p options
to provide access to the war/jar/yaml files and port 8080 (similar to what
the dobuild script does for Maven).

This sample project is provided without support, although the original
packager may respond to requests at jon.jarboe@contrastsecurity.com.
